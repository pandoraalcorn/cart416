...
public class BeatTracker : IDisposable
{
    ...
    public void FillStart()
    {
        ...
        for (float i = first - bestLength; i > bestLength; i -= bestLength)
        {
    ...
    {
        for (int i = 0; i < currentBeatLength; i++)
            offsetScore[i] = OffsetScore(upsampledSignal, currentBeatLength, i);       

        int offset = Util.GetBestIndex(offsetScore, 0, currentBeatLength);

        offset = currentBeatLength - offset;

        float d = offset - syncOld;
